...
#define NTHREADS 64	// number of threads per block
...
*/
void transpose(float *d_src, float *d_dest, uint width, int height)
{
    dim3 grid(iDivUp(width, BLOCK_DIM), iDivUp(height, BLOCK_DIM), 1);
    dim3 threads(BLOCK_DIM, BLOCK_DIM, 1);
    k_transpose<<< grid, threads >>>(d_dest, d_src, width, height);
...
// FLOAT version
void gaussianFilter(float *d_src, float *d_dest, float *d_temp, int width, int height, float sigma, int order)
{
    const float
    ...
    k_simpleRecursive<<< iDivUp(width, NTHREADS), NTHREADS >>>(d_src, d_temp, width, height, ema);
#else
    k_recursiveGaussian<<< iDivUp(width, NTHREADS), NTHREADS >>>(d_src, d_temp, width, height, a0, a1, a2, a3, b1, b2, coefp, coefn);
#endif
    CUT_CHECK_ERROR("Kernel execution failed");

    transpose(d_temp, d_dest, width, height);
    CUT_CHECK_ERROR("transpose: Kernel execution failed");

    // process rows
#if USE_SIMPLE_FILTER
    k_simpleRecursive<<< iDivUp(height, NTHREADS), NTHREADS >>>(d_dest, d_temp, height, width, ema);
#else
    k_recursiveGaussian<<< iDivUp(height, NTHREADS), NTHREADS >>>(d_dest, d_temp, height, width, a0, a1, a2, a3, b1, b2, coefp, coefn);
#endif
    CUT_CHECK_ERROR("Kernel execution failed");
